/**
 * bhoptimer-sfork - Paint
 * Original plugin by: SlidyBat
 *
...
public Action Timer_Paint( Handle timer )
{
	for( int i = 1; i <= MaxClients; i++ )
	{
		if( IsClientInGame( i ) && g_bIsPainting[i] )
		{
			static float pos[3];
			TraceEye( i, pos );
			...
			{
				AddPaint( i, pos, g_PlayerPaintColour[i], g_PlayerPaintSize[i] );
...
void AddPaint( int client, float pos[3], int paint = 0, int size = 0 )
{
	...
	// send to all clients if painter is admin
...
void CreatePaintMenus()
{
	...
	g_hPaintMenu.SetTitle( "Select Paint Color:" );
...
void SetClientPaintColour( int client, int paint )
{
	...
	Shavit_PrintToChat( client, "Paint color now: %s%s", gS_ChatStrings.sVariable, g_cPaintColours[paint][0] );
}
...
void SetClientPaintSize( int client, int size )
{
	...
	Shavit_PrintToChat( client, "Paint size now: %s%s", gS_ChatStrings.sVariable, g_cPaintSizes[size][0] );
}
